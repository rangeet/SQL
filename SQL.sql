...
--Q1. SELECT ALL STATES WHERE CUSTOMERS HAVE BOUGHT CELLPHONES FROM 2005 TILL TODAY.
	  ...
	  SELECT DISTINCT(T1.STATE) FROM DIM_LOCATION T1 
...
--Q4. SHOW THE CHEAPEST CELLPHONE
 ...
 --Q5. FIND OUT THE AVERAGE PRICE FOR EACH MODEL IN THE TOP 5 MANUFACTURERS IN TERMS OF SALES QUANITY
 --    AND ORDER BY AVERAGE PRICE?

		SELECT T1.MODEL_NAME, round(AVG(T2.TOTALPRICE),2) AS AVERAGE_PRICE 
			FROM DIM_MODEL T1 LEFT JOIN FACT_TRANSACTIONS T2 
				ON T1.IDModel = T2.IDModel
				LEFT JOIN DIM_MANUFACTURER T3
					ON T1.IDManufacturer = T3.IDManufacturer 
 ...
 --Q6. LIST THE NAME OF CUSTOMERS AND AVERAGE AMOUNT SPEND IN 2009, WHERE THE AVERAGE IS HIGHER THAN 500. 
		...
		SELECT T1.CUSTOMER_NAME, AVG(T2.TotalPrice) AS AMT_SPENT 
		FROM 
		DIM_CUSTOMER T1 
 ...
 --Q7. LIST IF THERE IS ANY MODEL THAT WAS IN TOP 5 IN TERMS OF QUANTITY , SIMULTANEOSLY  IN 2008,2009 and 2010 
		...
		(
			SELECT * FROM 
					(SELECT TOP 5 T1.Model_Name FROM DIM_MODEL T1 LEFT JOIN FACT_TRANSACTIONS T2 ON T1.IdModel = T2.IdModel 
																WHERE YEAR(DATE) = 2008
																GROUP BY T1.Model_Name
																ORDER BY SUM(T2.Quantity) DESC)X
 ...
 --Q8. SHOW THE MANUFACTURER WITH 2ND TOP SALES IN THE YEAR OF 2009 AND THE MANUFACTURER
 --    WITH THE 2ND TOP SALES IN 2010.
 	

		SELECT DISTINCT(YEAR(T3.DATE)) AS YEAR, T1.MANUFACTURER_NAME
		FROM DIM_MANUFACTURER T1 LEFT JOIN DIM_MODEL T2 ON T1.IdManufacturer = T2.IdManufacturer
								 LEFT JOIN FACT_TRANSACTIONS T3 ON T2.IdModel = T3.IdModel
		WHERE 
		(YEAR(T3.DATE) = 2009 )
			  AND T3.TotalPrice =  (SELECT MAX(TotalPrice) 
 ...
 --Q9. SHOW THE MANUFACTURERS WHO SOLD CELLPHONE IN 2010 BUT DIDNT IN 2009.

	SELECT DISTINCT(T1.MANUFACTURER_NAME) 
		...
		WHERE YEAR(T3.DATE) = 2010 AND YEAR(T3.DATE) != 2009

 --Q10. FIND TOP 100 CUSTOMERS AND AND THEIR AVERAGE SPEND, AVERAGE QUANTITY
 --     BY EACH YEAR. ALSO FIND THE PERCENTAGE IN THEIR SPEND
			
	  SELECT TOP 100 T1.CUSTOMER_NAME 
				...
				,	AVG(CASE WHEN YEAR(T2.DATE) = 2016 THEN (T2.QUANTITY) END) AS AVG_QUANTITY_IN_2006
